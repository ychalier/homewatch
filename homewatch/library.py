...
def probe_video(path: pathlib.Path, hidden_directory: str) -> dict:
    logger.info("Probing video at %s", path)
    probe_path = path.parent / hidden_directory / (path.stem + ".probe.json")
...
def extract_thumbnail(path: pathlib.Path, hidden_directory: str, width: int, height: int, duration: float) -> pathlib.Path:
    logger.info("Extracting thumbnail of %s (duration is %f)", path, duration)
    thumbnail_path = path.parent / hidden_directory / (path.stem + ".thumbnail.jpg")
    ...
    if process.returncode != 0:
        ...
        if duration > 0:
            ...
            return extract_thumbnail(path, hidden_directory, width, height, 0)
...
class LibraryEntry:
    ...
    def __init__(self, settings: Settings, folder: "LibraryFolder", basename: str):
...
class Media(LibraryEntry):
    ...
    def __init__(self,
        ...
        LibraryEntry.__init__(self, settings, folder, basename)
    ...
    def is_castable(self) -> bool:
        ...
        """
        if self.settings.chromecast_generation == ChromecastGeneration.NONE:
            return False
        if self.audio_codec is not None and not (
            self.audio_codec in {"flac", "aac", "mp3", "opus", "vorbis", "wav", "webm"}):
            return False
        if self.resolution is None or self.framerate is None or self.video_level is None:
            return False
        match self.settings.chromecast_generation:
            case ChromecastGeneration.GEN1 | ChromecastGeneration.GEN2:
            ...
            case ChromecastGeneration.GEN3:
            ...
            case ChromecastGeneration.ULTRA:
                return (self.video_codec == "h264" and self.video_level <= 42)\
    ...
    @classmethod
    def from_dict(cls, settings: Settings, folder: "LibraryFolder", d:dict):
        return cls(
            settings,
            folder,
            d["basename"],
    ...
    @classmethod
    def from_path(cls, settings: Settings, folder: "LibraryFolder", path: pathlib.Path):
        logger.debug("Analyzing media at %s", path)
        probe = probe_video(path, settings.hidden_directory)
        media = cls(settings, folder, path.name, float(probe["format"]["duration"]))
        ...
        media.thumbnail = extract_thumbnail(
            path,
            settings.hidden_directory,
...
class Folder(LibraryEntry):
    ...
    @classmethod
    def from_dict(cls, settings: Settings, folder: "LibraryFolder", d: dict):
        return cls(settings, folder, d["basename"])

    @classmethod
    def from_path(cls, settings: Settings, folder: "LibraryFolder", path: str | pathlib.Path):
        logger.debug("Analyzing subfolder at %s", path)
        return cls(settings, folder, os.path.basename(path))
...
class Playlist(LibraryEntry):
    ...
    @classmethod
    def from_dict(cls, settings: Settings, folder: "LibraryFolder", d: dict):
        return cls(settings, folder, d["basename"], d["elements"])
    ...
    def from_path(cls, settings: Settings, folder: "LibraryFolder", path: str | pathlib.Path):
        ...
        return cls(settings, folder, os.path.basename(path), elements)
...
class LibraryFolder:
    ...
    @classmethod
    def from_scan(cls, settings: Settings, root: str | pathlib.Path, path: pathlib.Path, quiet: bool = True):
        """
        @param document_root: library document root,
        """
        folder = cls(settings, path)
        ...
        for dirname in dirs:
            ...
            if dirname == settings.hidden_directory:
                continue
            folder.add_subfolder(Folder.from_path(settings, folder, dirname))
        for filename in files:
            ...
            if ext in settings.video_exts:
                media = Media.from_path(settings, folder, path)
                ...
                folder.add_playlist(Playlist.from_path(settings, folder, path))
        pbar.close()
    ...
    @classmethod
    def from_url(cls, settings: Settings, url: str):
        logger.info("Fetching library folder at %s", url)
        d = requests.get(url).json()
        return cls.from_dict(settings, d)

    @classmethod
    def from_dict(cls, settings: Settings, d: dict):
        folder = cls(settings, pathlib.Path(d["path"]))
        for dd in d["medias"]:
            folder.add_media(Media.from_dict(settings, folder, dd))
        for dd in d["subfolders"]:
            folder.add_subfolder(Folder.from_dict(settings, folder, dd))
        for dd in d["playlists"]:
            folder.add_playlist(Playlist.from_dict(settings, folder, dd))
        return folder

    @classmethod
    def from_file(cls, settings: Settings, path: str):
    ...
    @classmethod
    def from_settings(cls, settings: Settings, path: pathlib.Path):
        if settings.library_mode == "local":
            return cls.from_scan(settings, pathlib.Path(settings.library_root), path)
        elif settings.library_mode == "remote":
            url = urllib.parse.urljoin(settings.library_root, (path / "index.json").as_posix())
            return cls.from_url(settings, url)
...
class Hierarchy:
    ...
    @classmethod
    def from_scan(cls, settings: Settings, root: pathlib.Path):
    ...
    def from_settings(cls, settings: Settings):
        if settings.library_mode == "local":
            return cls.from_scan(settings, pathlib.Path(settings.library_root))
        elif settings.library_mode == "remote":
            return cls.from_url(settings.library_root)
        raise ValueError(f"LIBRARY_MODE: {settings.library_mode}")


def test_library_connection(library_mode: Literal["local"] | Literal["remote"], library_root: str, max_retries: int = 3):
    if library_mode == "local":
        return
    for attempt in range(max_retries):
        try:
            response = requests.get(library_root + "alive")
...
class Library(dict[str, LibraryFolder]):
    ...
    def __init__(self, settings: Settings, root: str | pathlib.Path, folders: list[LibraryFolder] = []):
        self.settings = settings
        self.root = root
    ...
    def from_dict(cls, settings: Settings, d: dict):
        return cls(
            settings,
            pathlib.Path(d["root"]),
            [LibraryFolder.from_dict(settings, dd) for dd in d["folders"]])

    @classmethod
    def from_scan(cls, settings: Settings):
        root = pathlib.Path(settings.library_root)
        logger.info("Scanning library at %s", root)
        if not root.is_dir():
            logger.error("Library root does not exist: %s", root)
            raise FileNotFoundError(str(root))
        library = cls(settings, root)
        hierarchy = Hierarchy.from_settings(settings)
        ...
        for folder in hierarchy.folders:
            ...
            library_folder = LibraryFolder.from_scan(settings, library.root, folder_path, True)
    ...
    @classmethod
    def from_url(cls, settings: Settings):
        url = settings.library_root
        logger.info("Fetching library at %s", url)
        test_library_connection(settings.library_mode, settings.library_root)
        library = cls(settings, url)
        hierarchy = Hierarchy.from_settings(settings)
        total = sum([folder.medias for folder in hierarchy.folders])
        ...
        for folder in hierarchy.folders:
            ...
            library_folder = LibraryFolder.from_url(settings, folder_url)
            library[folder_path.as_posix()] = library_folder
            pbar.update(folder.medias)
        pbar.close()
        return library
    ...
    def from_settings(cls, settings: Settings):
        if settings.library_mode == "local":
            return cls.from_scan(settings)
        elif settings.library_mode == "remote":
            return cls.from_url(settings)
        raise ValueError(f"LIBRARY_MODE: {settings.library_mode}")

    @staticmethod
    def clear_hidden_directories(top: pathlib.Path, hidden_directory: str):
        logger.info("Clearing library at %s", top)
        for path in list(top.glob("**")):
            if path.is_dir() and path.name == hidden_directory:
